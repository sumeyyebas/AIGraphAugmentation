...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    rest_graphs = self.graphs[:self.num_train]
    ### shuffle all graphs
    if self.is_shuffle:
      self.npr = np.random.RandomState()
      self.npr.shuffle(rest_graphs)

    self.graphs_train = rest_graphs[:self.num_train]
    self.graphs_dev = rest_graphs[:self.num_dev]
  ...
  def test(self):
    ...
    if self.config.test.is_test_ER:
      ...
      file_path = 'graphs_gen.pkl'
      with open(file_path, 'wb') as file:
          pickle.dump(graphs_gen, file)
